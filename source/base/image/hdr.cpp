...
Image *Read(IStream *file, const Image::ReadOptions& options)
{
    ...
    while(*file)
    {
        if((file->getline(line, sizeof(line)) == NULL) || (line[0] == '-') || (line[0] == '+'))
            break;
    ...
    for(int row = 0; row < height; row++)
    {
        if((width < MINELEN) | (width > MAXELEN))
        {
            for(int col = 0; col < width; col++)
            {
                ...
                if(file->write(&rgbe, sizeof(RGBE)) == NULL)
            ...
            for(int i = 0; i < 4; i++)
            {
                for(int col = 0; col < width; col += cnt)
                {
                    ...
                    if(beg - col > 1 && beg - col < MINRUN)
                    {
                        ...
                        while(scanline[c2++][i] == scanline[col][i])
                        {
                            if(c2 == beg)
                            {
                                ...
                                if(file->Write_Byte(scanline[col][i]) == NULL)
                                    throw POV_EXCEPTION(kFileDataErr, "Failed to write data to HDR file");
                    ...
                    if(cnt >= MINRUN)
                    {
                        ...
                        if(file->Write_Byte(scanline[beg][i]) == NULL)
                            throw POV_EXCEPTION(kFileDataErr, "Failed to write data to HDR file");
                    }
                    else
                        cnt = 0;
